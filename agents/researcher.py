...
def _pubmed_search(topic, start, limit):
    rows = []
    try:
        p = requests.get(
            ...
            params={
                'db': 'pubmed',
                'term': f'({topic}) AND ({start}:3000[pdat])',
                'retmode': 'json',
                'retmax': limit,
                'sort': 'relevance'
            },
        ...
        for pid in ids:
            ...
            rows.append({
                'title': x.get('title', ''),
                'year': str(x.get('pubdate', ''))[:4],
                'authors': ', '.join(a.get('name', '') for a in x.get('authors', [])[:8]),
                'doi': '',
                'pmid': pid,
                'source': 'PubMed',
...
def _openalex_search(topic, start, limit):
    """OpenAlex is optional. Failures are swallowed so the pipeline can continue with PubMed."""
    rows = []
    mailto = os.getenv('OPENALEX_EMAIL', '') or 'daily-paper-writer@users.noreply.github.com'
    for attempt in range(2):
        try:
            ...
            for x in p.json().get('results', []):
                ...
                pdf = loc.get('pdf_url', '') if loc else ''
                rows.append({
                    'title': x.get('display_name', ''),
                    'year': str(x.get('publication_year', '')),
                    'authors': ', '.join((a.get('author') or {}).get('display_name', '') for a in x.get('authorships', [])[:8]),
                    'doi': doi,
                    'pmid': '',
                    'source': 'OpenAlex',
            ...
            print(f'OpenAlex search skipped (optional): {e}')
            time.sleep(2)
    return rows
...
def search(topic, years=5, limit=20):
    ...
    rows = []
    rows.extend(_pubmed_search(topic, start, limit))
    rows.extend(_openalex_search(topic, start, limit))
...
def download_pdfs(rows, folder):
    ...
    for i, r in enumerate(rows, 1):
        ...
        if not url:
            continue
        try:
            resp = requests.get(url, timeout=60, headers={'User-Agent': 'DailyPaperWriter/1.0'})
            ...
            path = Path(folder) / f'{i:03d}_{safe}.pdf'
